...
bool TTFFont::reload(
    ...
    if (uint8_t* d = s->get_pointer())
        stbtt_InitFont(&font, (const unsigned char*)d, 0);
    else {
...
TTFFont::clone_type(const std::string& arguments) const {
...
void GlyphCache::build_distance_field(uint8_t* bu,
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        for (int y = 2; y < kGlyphRenderRes - 2; ++y) {
            for (int x = kGlyphRenderRes - 2; x > 2; --x)
                set_val(x, y, bu);
        }
        for (int y = kGlyphRenderRes - 2; y > 2; --y) {
            for (int x = 1; x < kGlyphRenderRes - 2; ++x)
...
void Label::render_str(const char* string,
    ...
    for (const Line& line : lines) {
        ...
        if (style == kLineCenter)
            offset = -line.width * 0.5;
        else if (style == kLineRight)
            offset = -line.width;
