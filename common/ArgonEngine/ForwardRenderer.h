...
class ForwardRenderer {
  ...
  public:
    ...
    std::vector<Vector4f>& light_positions =
        renderer_uniforms.f4["light_positions"];
    std::vector<Vector4f>& light_colors =
        renderer_uniforms.f4["light_colors"];

    std::vector<Vector4f>& light_radius =
        renderer_uniforms.f4["light_radius"];
    std::vector<Vector4f>& light_power =
        renderer_uniforms.f4["light_power"];
    Vector3f& camera_pos =
        renderer_uniforms.get_fvec3("camera_pos");

    float& frame_time =
        renderer_uniforms.get_scalar("time");
    Matrix4f& view_matrix =
        renderer_uniforms.get_mat4("view_matrix");
    Matrix4f& projection_matrix =
        renderer_uniforms.get_mat4("projection_matrix");
    float& total_lights =
        renderer_uniforms.get_scalar("total_lights");
    Vector2f& pixel_size =
        renderer_uniforms.get_fvec2("pixel_size");
    Vector2f& viewport =
        renderer_uniforms.get_fvec2("viewport");
    VirtualResource fallback_shader;
    VirtualResource& noise_texture =
        renderer_uniforms.get_texture("noise_texture");
    ...
    ForwardRenderer() {
        ...
        total_lights = 0;
        noise_texture =
            "virtual://noise-texture.tex{w=h=128; "
            "f=RGBA8;mip=0;}";

        fallback_shader = "shader://fallback.shd";

        VirtualResourceImage* noise =
            noise_texture.get_data<VirtualResourceImage*>();
        for (int x = 0; x < 128; ++x) {
            for (int y = 0; y < 128; ++y) {
                Vector4f f((rand() % 1000) * 0.001,
                           (rand() % 1000) * 0.001,
                           (rand() % 1000) * 0.001,
                           (rand() % 1000) * 0.001);
    ...
    ShaderData& get_shader_data(VirtualResource& r) {
        ...
        if (!d.full_color)
            d.full_color = "append:{shader://default.shd," +
                           r.get_path_string() + "}";

        if (!d.full_depth)
            d.full_depth = "append:{shader://"
                           "default.shd,string:{#define "
                           "DEPTH_ONLY 1 \n}," +
                           r.get_path_string() + "}";
    ...
    void do_shader(std::shared_ptr<Material> m) {
        ...
        if (!m->shader)
            m->shader = fallback_shader;
    }

    // do_shader
    static bool sort_primitives(Renderable* a,
                                Renderable* b) {
        if (a->layer != b->layer)
    ...
    void draw(RenderAPI& api) {
        ...
        viewport =
            Vector2f(Argon::Screen::framebuffer_size[0],
                     Argon::Screen::framebuffer_size[1]);
        pixel_size =
            Vector2f(1. / Argon::Screen::logical_size[0],
                     1. / Argon::Screen::logical_size[1]);
        ...
        while (x % 4) {
            light_positions[x] =
                Vector4f(INFINITY, INFINITY, INFINITY, 0.1);
        ...
        projection_matrix =
            Camera::get_main_camera()->projection_matrix;
        ...
        renderer_uniforms.get_mat4("view_normal") =
            transpose(inv_c);
        ...
        api.set_clear_color(
            Camera::get_main_camera()->background_color);
        ...
        while (b_it) {
            if (b_it->parent) {
                BoundingCube c =
                    b_it->parent->bounds.transform(
                        b_it->parent->world_matrix());
                if (bounds_count >= bounds.size()) {
                    std::shared_ptr<BoundingCubeNode> n =
                        std::make_shared<
                            BoundingCubeNode>();
                    ;
                    bounds.push_back(n);
                ...
                bounds[bounds_count]->layer =
                    b_it->parent->layer;
                bounds[bounds_count]->render_flags &=
        ...
        while (current) {
            if (current->material &&
                current->vertex_array) {
                current->update_transform();
                BoundingCube c = current->bounds.transform(
                    current->last_world_matrix);
                int render_flags =
                    current->render_flags &
                    current->material->render_flags;
                if (render_flags & kRenderShouldRender &&
                    !f.should_cull(c)) {
        ...
        while (it != all_visible.end()) {
            ...
            if ((current->get_layer() & kRenderLayerMask) ==
                    0 &&
                !current->is_transparent()) {
                ShaderData& cs = get_shader_data(
                    current->material->shader);
                Uniforms* unif[] = {
                    &(current->uniform),
                    &(current->material->uniform),
                    &renderer_uniforms};

                api.set_shader(current, cs.full_color, unif,
                               3);
                int render_flags =
                    current->render_flags &
                    current->material->render_flags;
                ...
                api.draw_vertex_array(
                    current->vertex_array,
                    current->end_vertex,
                    current->vertex_array->draw_type);
            }
            ++it;
        }
        ...
        projection_matrix =
            Camera::get_main_camera2D()->projection_matrix;
        it = all_visible.begin();
        while (it != all_visible.end()) {
            Renderable* current = *it;
            if ((current->get_layer() & kRenderLayerMask) >
