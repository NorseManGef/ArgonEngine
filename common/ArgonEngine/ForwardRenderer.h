...
class ForwardRenderer {
  ...
  public:
    ...
    ForwardRenderer() {
        ...
        total_lights = 0;
        noise_texture = "virtual://noise-texture.tex{w=h=128; "
                        "f=RGBA8;mip=0;}";
        ...
        for (int x = 0; x < 128; ++x) {
            for (int y = 0; y < 128; ++y) {
                Vector4f f((rand() % 1000) * 0.001, (rand() % 1000) * 0.001,
                           (rand() % 1000) * 0.001, (rand() % 1000) * 0.001);
    ...
    ShaderData& get_shader_data(VirtualResource& r) {
        ...
        if (!d.full_color)
            d.full_color =
                "append:{shader://default.shd," + r.get_path_string() + "}";
    ...
    void draw(RenderAPI& api) {
        ...
        viewport = Vector2f(Argon::Screen::framebuffer_size[0],
                            Argon::Screen::framebuffer_size[1]);
        pixel_size = Vector2f(1. / Argon::Screen::logical_size[0],
        ...
        while (current) {
            if (current->material && current->vertex_array) {
                ...
                BoundingCube c =
                    current->bounds.transform(current->last_world_matrix);
                int render_flags =
                    current->render_flags & current->material->render_flags;
                if (render_flags & kRenderShouldRender && !f.should_cull(c)) {
                    do_shader(current->material);
                    api.cache_array(current->vertex_array);
                    all_visible.insert(
                        std::lower_bound(all_visible.begin(), all_visible.end(),
                                         current, sort_primitives),
                        current);
                }
            }
        ...
        while (it != all_visible.end()) {
            ...
            if ((current->get_layer() & kRenderLayerMask) == 0 &&
                ...
                api.draw_vertex_array(current->vertex_array,
                                      current->end_vertex,
