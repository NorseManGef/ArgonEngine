...
/**
 * @brief This file contains classes for a basic
 * high-score/achievement system.
...
/**
 * @brief An implementation of a basic
 * high-score/achievement system.
 * @details
 * This class provides many common high score related
 * functionality. And also forms an interface with global
 * high-score systems handled by the HAL.
 *
 * It provides functionality for:
 * - Loading, Saving, Pushing, and Pulling scores from the
 * disk and online high score systems.
 * - Signalling to the HAL the display of platform specific
 * leader boards/achievement systems.
...
class ScoreController : public ReflectionBase {
  ...
  public:
    ...
    //! Gets the users highest global score in leaderboard
    //! 's'.
    static ScoreControllerScore&
    get_global_ranking(const Argon::StringIntern& s) {
    ...
    //! Returns true if the HAL should display the
    //! leaderboards.
    static bool should_display_leaderboard() {
        bool b = displayLeaderboard;
        displayLeaderboard = false;
        return b;
    }
    //! Returns true if the HAL should display an
    //! achievements modal.
    static bool should_display_achievments() {
        bool b = displayAchievments;
        displayAchievments = false;
        return b;
    }
    //! Returns a vector of all achievements that have not
    //! been submitted.
    static const std::vector<ScoreControllerAchievement>&
    get_unsubmitted_achievements() {
    ...
    //! Clears the vector of all unsubmitted scores. Call
    //! this once the HAL has uploaded all unsubmitted
    //! scores.
    static void clear_unsubmitted_scores();

    //! Clears the vector of all unsubmitted achievements.
    //! Call this once the HAL has uploaded all unsubmitted
    //! achievements.
