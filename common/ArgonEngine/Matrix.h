...
template <typename T, size_t TSize> struct MatrixBase {
    ...
    INLINE VectorBase<T, TSize> row(size_t r) const {
        ...
        for (size_t x = 0; x < TSize; ++x)
            v[x] = operator[](x + r * TSize);
        return v;
    }
    INLINE VectorBase<T, TSize> col(size_t c) const {
        VectorBase<T, TSize> v;
        for (size_t x = 0; x < TSize; ++x)
            v[x] = operator[](x * TSize + c);
        return v;
    }
};
template <typename T>
...
template <typename T> struct TypeInfo<MatrixBase4<T>> {
    enum { valid = 1 };
    static const char* get_name() { return "Matrix4"; }
    static void handle(MatrixBase4<T>& v, Visitor& vis) {
        for (int x = 0; x < 16; ++x)
            vis.handle(v[x]);
    }
};
template <typename T> struct TypeInfo<MatrixBase3<T>> {
    enum { valid = 1 };
    static const char* get_name() { return "Matrix3"; }
    static void handle(MatrixBase3<T>& v, Visitor& vis) {
        for (int x = 0; x < 9; ++x)
            vis.handle(v[x]);
    }
};
template <typename T> struct TypeInfo<MatrixBase2<T>> {
    enum { valid = 1 };
    static const char* get_name() { return "Matrix4"; }
...
template <typename T> struct ScaleMatrixCookie {
    VectorBase<T, 3> data;
    ScaleMatrixCookie(const VectorBase<T, 3> d) : data(d) {}
};
