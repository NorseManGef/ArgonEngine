...
struct InstancedLines : public Node {
    ...
    InstancedLines() {
        material->shader =
            "shader://instanced_line_shader.shd";
    }
    void set_lines(size_t total_liness) {
        int total_renders =
            total_lines / kLinesPerBatch + 1;
        while (arrays.size() < total_renders) {
            arrays.push_back(
                std::make_shared<VertexArray>());
            arrays.back()->add_attribute<float>(
                3, kPositionAttribute);
            arrays.back()->add_attribute<float>(
                4, kColorAttribute);
        ...
        if (arrays.size() != total_renders) {
            arrays.resize(total_renders);
        }
        ...
        while (renders.size() < total_renders) {
            ...
            render->vertex_array =
                arrays[renders.size() - 1];
        }
        if (renders.size() != total_renders)
            renders.resize(total_renders);
        size_t size_r = total_lines;
        int index = 0;
        while (size_r >= kLinesPerBatch) {
            if (arrays[index]->vertex_count() !=
                kVerticesPerBatch) {
    ...
    void set_trail(size_t index, const Vector3f& v,
                   const Vector4f& c) {
        VertexArray& it = *arrays[index >> 14ul];
        float* d = ((float*)it.data_start()) +
                   (index & kSpritesMask) * 14;
        d[0] = d[7];
        d[1] = d[8];
        d[2] = d[9];
    ...
    void set_line(size_t index, const Vector3f& v0,
                  const Vector3f& v1, const Vector4f& c0,
                  const Vector4f& c1) {
        VertexArray& it = *arrays[index >> 14ul];
        float* d = ((float*)it.data_start()) +
                   (index & kSpritesMask) * 14;
    ...
    void set_transform(size_t index, Transform& transform) {
        if (transform.get_should_render()) {
            ...
            Vector4f pos[2] = {Vector4f(-0.5, 0.0, 0, 1),
                               Vector4f(0.5, 0, 0, 1)};
            pos[0] = world_matrix * pos[0];
            pos[1] = world_matrix * pos[1];
            Vector4f c = transform.get_color();

            set_line(index, Vector3f(pos[0] / pos[0][3]),
                     Vector3f(pos[1] / pos[1][3]), c, c);
        } else {
            set_line(index, Vector3f(0, 0, 0), Vector3f(),
