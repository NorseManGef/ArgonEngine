...
#define DEFINE_VEC_OPERATOR(op)                                                \
    ...
    INLINE VectorBase<T, TSize> operator op(const T& a,                        \
    ...
    template <typename T, size_t TSize>                                        \
    INLINE VectorBase<T, TSize> operator op(const VectorBase<T, TSize> a,      \
                                            const T& b) {                      \
        VectorBase<T, TSize> v;                                                \
        for (size_t x = 0; x < TSize; ++x)                                     \
            v[x] = a[x] op b;                                                  \
    ...
    INLINE VectorBase<typename TypePromotion<T, T2>::type, TSize> operator op( \
        const VectorBase<T, TSize>& a, const VectorBase<T2, TSize> b) {        \
...
template <typename T, size_t TSize> class VectorBase {
  public:
    ...
    VectorBase operator+(const T& v) const { return VectorBase(*this) += v; }
    VectorBase operator-(const T& v) const { return VectorBase(*this) -= v; }
    VectorBase operator-() const { return VectorBase(*this) * -1.0; }
    VectorBase operator*(const T& v) const { return VectorBase(*this) *= v; }
    VectorBase operator/(const T& v) const { return VectorBase(*this) /= v; }
...
template <typename T> struct VectorBase3 : public VectorBase<T, 3> {
    ...
    template <typename T2> VectorBase3(const VectorBase<T2, 3>& v) {
        for (size_t s = 0; s < 3; ++s)
            this->data[s] = v[s];
    }
    VectorBase3(const T x, const T y, const T z) { set(x, y, z); }
...
template <typename T> struct VectorBase4 : public VectorBase<T, 4> {
    ...
    VectorBase4(const T x, const T y, const T z, const T w) { set(x, y, z, w); }
...
template <typename T, size_t size> struct TypeInfo<VectorBase<T, size>> {
    enum { valid = 1 };
...
typename TypePromotion<T1, T2>::type dot(const VectorBase<T1, TSize>& a,
...
T distance(const VectorBase<T, TSize>& a, const VectorBase<T2, TSize>& b) {
    return length<T, TSize>(a - b);
}
//! Returns a vector of unit length that is in the same
//! direction as vector a.
template <typename T, size_t TSize>
VectorBase<T, TSize> normalize(const VectorBase<T, TSize>& a) {
...
const static Vector4f kGreenColor(0, 1, 0,
                                  1); //!< Holds a Vector representation of
                                      //!< the color green.
const static Vector4f kBlueColor(0, 0, 1,
                                 1); //!< Holds a Vector representation of the
                                     //!< color blue.

const static Vector4f kYellowColor(1, 1, 0,
                                   1); //!< Holds a Vector representation of
                                       //!< the color yellow.
const static Vector4f kMagentaColor(1, 0, 1,
                                    1); //!< Holds a Vector representation of
                                        //!< the color magenta.
const static Vector4f kCyanColor(0, 1, 1,
                                 1); //!< Holds a Vector representation of the
                                     //!< color cyan.

const static Vector4f kClearColor(0, 0, 0,
                                  0); //!< Holds a Vector representation of a
                                      //!< color that is completely transparent.
const static Vector4f kWhiteColor(1, 1, 1,
                                  1); //!< Holds a Vector representation of
                                      //!< the color white.
const static Vector4f kGrayColor(0.5f, 0.5f, 0.5f,
                                 1); //!< Holds a Vector representation of the
                                     //!< color gray.
const static Vector4f kBlackColor(0, 0, 0,
                                  1); //!< Holds a Vector representation of
                                      //!< the color black.

const static Vector4f kBrownColor(0.62f, 0.41f, 0.21f,
                                  1); //!< Holds a Vector representation of
                                      //!< the color brown.
const static Vector4f kPurpleColor(0.5f, 0, 0.5f,
                                   1); //!< Holds a Vector representation of
                                       //!< the color purple.
const static Vector4f kOrangeColor(1, 0.5f, 0,
                                   1); //!< Holds a Vector representation of
                                       //!< the color orange.

//! Performs the cross product of two 3 component vectors.

template <typename T, size_t comps> VectorBase<T, comps> random_vector() {
...
template <typename T>
static inline VectorBase<T, 3> cross(const VectorBase<T, 3>& op1,
