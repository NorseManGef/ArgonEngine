...
/**
 * @brief This file provides some utilities for template
 * meta-programming.
...
namespace Argon {
/**
 * @brief This class is used to rank the precision of
 * various intrinsic types, to determine the conversion
 * order. The rank value is higher if the variable holds
 * more precision.
 * @tparam T The type to rank.
 **/
template <typename T> struct TypePromotionRank;
template <> struct TypePromotionRank<bool> {
    enum { rank = 0 };
};
template <> struct TypePromotionRank<uint8_t> {
    enum { rank = 1 };
};
template <> struct TypePromotionRank<int8_t> {
    enum { rank = 2 };
};
template <> struct TypePromotionRank<uint16_t> {
    enum { rank = 3 };
};
template <> struct TypePromotionRank<int16_t> {
    enum { rank = 4 };
};
template <> struct TypePromotionRank<uint32_t> {
    enum { rank = 5 };
};
template <> struct TypePromotionRank<int32_t> {
    enum { rank = 6 };
};
template <> struct TypePromotionRank<uint64_t> {
    enum { rank = 7 };
};
template <> struct TypePromotionRank<int64_t> {
    enum { rank = 8 };
};
template <> struct TypePromotionRank<float> {
    enum { rank = 9 };
};
template <> struct TypePromotionRank<double> {
    enum { rank = 10 };
};
template <> struct TypePromotionRank<long double> {
    enum { rank = 11 };
};
...
/**
 * @brief This class uses the TypePromotionRank to determine
 * the necessary return type of an operation so that
 * precision will not be lost, and typedefs type to the
 * necessary type.
