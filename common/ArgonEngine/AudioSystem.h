...
/**
 * @brief This file includes most of the code for the Audio
...
/// This is the base class of all the Audio
/// effect/generators.
struct AudioNode : virtual public ReflectionBase {
    ...
    /// Called to Render audio samples into the buffer,
    /// additively.
    virtual void render(float* buffer) {}
    MAKE_VISIT(AudioNode, { ADD_VAR(volume); })
};
...
/// A node that additively mixes an arbitrary amount of
/// channels.
...
/// A node that duplicates the output of one AudioNode into
/// another.
...
struct OggNode {
    ...
    static int op_seek(void* stream, opus_int64 offset,
        ...
        if (whence == SEEK_SET)
            ...
            data->offset = data->resource.size() - offset;
        else
            return -1;
    ...
    OggNode() {
        ...
        for (int x = 0; x < 8; ++x) {
            ...
            channels[x]->input = clones[x];
            clones[x]->parent = this;
        }
        left = channels[0];
        right = channels[1];
        opus_file = NULL;
...
struct AudioSource3D : public ReflectionBase {
    ...
    Vector3f position;
    /// The last position of the source, this is updated
    /// automatically.
...
/// An AudioNode that implements 3D Audio, by bipolar phase
/// inteferance.
struct Listener : public AudioNode {
    ...
    /// The last position of the listener, updated
    /// automatically.
