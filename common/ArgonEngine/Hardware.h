...
class SleepLock {
  ...
  public:
    SleepLock() : have_lock_(false) { prevent_sleep(); }
    void prevent_sleep(); // stops device from sleeping
    void allow_sleep();   // allows the device to sleep
    static bool sleep_enabled() { return !locks_; }
    ~SleepLock() { allow_sleep(); }
};
...
/// The InputState struct contains useful data and derived
/// data for an input source.
struct InputState : public ReflectionBase {
    /// The current value of the input.
    double value = 0;
    /// The current magnitude of the inputs motion.
    /// This is useful when you want to know things like how
    /// fast the user is pressing a button, or moving a
    /// joystick back and forth.
    double magnitude = 0;
    /// How quickly the magnitude decays between 1 and 0,
    /// the closer it is to 0 the faster it decays.
    double magnitude_decay = 0.9;
    /// The integral of the input. Useful for things like
    /// finding out how long a button has been pressed.
    double deadzone = 0.0;
    double integral = 0;
    /// The low-pass filtered value of the input.
    double filtered_value = 0;
    /// The coefficient for the IIR low-pass filter, between
    /// 1 and 0.
    double filter_ratio = 0.95;
    /// The timestamp of the last update.
    double last_time = 0;
    /// An offset that is added to the actual value before
    /// all other variables are computed. This is useful for
    /// providing calibration features.
    double offset = 0;
...
/**
 * @breif The Input class allows you to get data from
 * various input devices such as mice, keyboards, joysticks,
 * etc., and various information from the HAL.
 *
 * @details Each Input object spawned, and all of its input
 * states are completely seperate from each other. This
 * allows you to make modifications to the InputState of an
 * input device, and calculate things like the magnitude,
 * without effecting the values of the same input read using
 * a different Input object. Which helps keep the code very
 * modular.
 **/
struct Input {
    /// The global input ring-buffer. If the buffer
    /// overflows between input updates the state changes
    /// are lost.
    static InputUpdate* buffer() {
        static InputUpdate i[512];
        return i;
    }
    /// The current global buffer index.
    static RollingInt<0, 511>& buffer_index() {
        static RollingInt<0, 511> x = 0;
        return x;
    }
    ...
    InputUpdate get_last_update(double min_update = 0.5,
                                double max_time_ago = 3.);

    /// Updates the derived values for all observed input
    /// devices of this object.
    void update_values();
...
#endif
        if (values[inp].last_time == 0. &&
            (input_id & kInputSourceMask) == kInputJoy)
