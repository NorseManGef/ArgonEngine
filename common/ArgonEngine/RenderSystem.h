...
/**
 * @brief This file declares all the classes used in the
 * Render System.
 * @file RenderSystem.h
 * @author Skyler Saleh
 **/
...
namespace Argon {
/**
 * @brief This class is used to implement a camera in a
 * scene.
 * @details
 * Each active camera can have multiple render targets.
 * And each render target can only belong to one camera.
 * Every camera that has a render target that is accessed
 * when rendering the main camera is rendered to.
 *
 * Regular Render to Texture functionality can be
 * implemented by setting a cameras render target to a
 * Texture(.tex)
 *
 * While rendering with a specific renderer can be done by
 * setting the target to framebuffer(.fbo)
 *
 * To render to the screen you use the Render Target
...
template <> struct ToRenderType<float> {
    enum { type = kRenderTypeFloat };
};
template <> struct ToRenderType<uint8_t> {
    enum { type = kRenderTypeUByte };
};
template <> struct ToRenderType<int16_t> {
    enum { type = kRenderTypeShort };
};
template <> struct ToRenderType<int8_t> {
    enum { type = kRenderTypeByte };
};
//! @brief This class is used to represent a renderable mesh
//! in the RenderSystem.
struct VertexArray : public ReflectionBase {

    //! A tag used for debug purposes.
    StringIntern tag;
    //! A hint to the RenderSystem to optimize for frequent
    //! updates.
    ...
    //! @details This is used to map vertex data to
    //! vertices, so vertices can be shared.
    ...
    //! @details Increment this when the data in the array
    //! has been changed.
    int update_id;
    ...
    ptrdiff_t stride;
    //! Recalculates the stride based on the added
    //! attributes.
    void recalc_stride();
    float prim_size = 5;

    VertexArray();
    //! Returns true if attribute 's' is present in the
    //! vertex array.
    bool has_attribute(const StringIntern& s);
    //! An iterator to the start of the data of attribute
    //! 's'
    VertexIterator begin(const StringIntern& s);
    //! An iterator to the end of the data of attribute 's'
    VertexIterator end(const StringIntern& s) {
        return begin(s) + vertex_count();
    }
    ...
    //! Append the data 'd' of size 'bytes_size' to the end
    //! of the array.
    void append_data(const uint8_t* d, int bytes_size);
    //! Sets the vertex array data by resizing the data to
    //! 'bytes_size' bytes, and copying the data from 'd'.
...
struct Light : public InstanceList<Light>, public Node {
    ...
    //! The lights falloff distance in meters. Larger values
    //! make the lights bounds larger.
    float distance = 10;
...
struct Uniforms : ReflectionBase {
    ...
    Vector4f& get_fvec4(const StringIntern& s) {
        auto& it = f4[s];
        if (it.empty())
            it.push_back(Vector4f(0, 0, 0, 0));
        return it[0];
    }
    Vector3f& get_fvec3(const StringIntern& s) {
        auto& it = f3[s];
        if (it.empty())
            it.push_back(Vector3f(0, 0, 0));
        return it[0];
    }
    Vector2f& get_fvec2(const StringIntern& s) {
        auto& it = f2[s];
        if (it.empty())
            it.push_back(Vector2f(0, 0));
        return it[0];
    }
    MAKE_VISIT_HEAD(Uniforms)
};

//! @brief This class represents the material information
//! when rendering.
struct Material : public ReflectionBase {
    ...
    //! @details
    //! Different Renderers require different shaders to
    //! work. So check the documentation of your chosen
    //! renderer to determine what shaders you can provide.
    VirtualResource shader = "shader://fallback.shd";

    //! @brief The blending style to use. This is a bitmask
    //! of Argon::BlendFactor
    unsigned int blend = kBlendReplace;

    //! @brief a map of the uniforms used in the material.
    //! @details
    //! This allows your application to provide information
    //! to your shaders as uniform variables. These are used
    //! to control the appearance of objects. Read the
    //! shaders source to see what values can be modified.
    Vector4f& color = uniform.get_fvec4("color");

    //! A tag used for debugging
    StringIntern name = "Unnamed Material";

    //! @brief The render flags to use when rendering. This
    //! is a bitfield of Argon::RenderFlags.
    //! @details
    //! This is bitwise AND'd with the Renderables
...
/**
 * @brief This class is used to create an object that is to
 * be rendered.
 * @details
 * The Renderable class is used to describe the rendering
 * attributes of an object in the ArgonEngine.
 *
 * Each Renderable serves a node to a Doubly Linked List,
 * which is used to iterate through all Renderables in a
 * scene. This means that the act of having a Renderable
...
struct Renderable : public InstanceList<Renderable>, public Node {
    ...
    //! @brief Flags that control the rendering of an object
    //! such as the associated masks.
    //! @details
    //! This is a bitmask of RenderFlags, and is bitwised
    //! AND'd with the materials variable of the same name
    //! to generate an objects render flags.
    int render_flags = kRenderDefault;
    StringIntern tag = "Unnamed Renderable";

    //! Set to the number of the last vertex to render from
    //! the vertex array, or -1 to render all vertices.
    int end_vertex = -1;
