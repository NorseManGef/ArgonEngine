...
Matrix4f Node::world_matrix() {
    return (parent ? parent->world_matrix() : IdentityMatrix<float>()) *
           TranslateMatrix(position) * RotateMatrix(rotation) *
           ScaleMatrix(dimensions * scale) * TranslateMatrix(anchor);
}

Matrix4f Node::object_matrix() {
    return IdentityMatrix<float>() * TranslateMatrix(position) *
           RotateMatrix(rotation) * ScaleMatrix(dimensions * scale) *
...
Matrix4f Node::get_camera_view() { return Camera::get_main_camera()->matrix; }
...
Matrix4f Node::view_matrix() {
    return (parent ? parent->view_matrix()
                   : Camera::get_main_camera()->matrix) *
           TranslateMatrix(position) * RotateMatrix(rotation) *
           ScaleMatrix(dimensions * scale) * TranslateMatrix(anchor);
}

Matrix4f Node::object_matrix_inverse() {
    return IdentityMatrix<float>() * TranslateMatrix(-anchor) *
           ScaleMatrix(1.0f / (dimensions * scale)) *
           RotateMatrix(rotation.inverse()) * TranslateMatrix(-position);
}
...
Matrix4f Node::view_matrix_inverse() {
    return IdentityMatrix<float>() * TranslateMatrix(-anchor) *
           ScaleMatrix(1.0f / (dimensions * scale)) *
           RotateMatrix(rotation.inverse()) * TranslateMatrix(-position) *
           (parent ? parent->view_matrix_inverse()
                   : Camera::get_main_camera()->inverse());
}

Matrix4f Node::object_normal_matrix() {
    return transpose(object_matrix_inverse());
}

Matrix4f Node::world_normal_matrix() {
    return transpose(world_matrix_inverse());
}
...
Vector3f Node::screen_to_local(Vector3f p) {
    ...
    Matrix4f m = Camera::get_main_camera()->projection_matrix *
                 Camera::get_main_camera()->matrix;
