...
Matrix4f Node::world_matrix() {
    return (parent ? parent->world_matrix()
...
Matrix4f Node::projection_matrix() {
    ...
    return (get_camera_projection() * get_camera_view()) *
           world_m;
}
...
Matrix4f Node::world_matrix_inverse() {
    return IdentityMatrix<float>() *
           ...
           RotateMatrix(rotation.inverse()) *
           TranslateMatrix(-position) *
           (parent ? parent->world_matrix_inverse()
                   : IdentityMatrix<float>());
}
...
Matrix4f Node::view_normal_matrix() {
    return transpose(view_matrix_inverse());
}
