...
class OpenGLES : public RenderAPI {
    struct vert_data {
        ...
        size_t last_frame;
        vert_data()
            : vert_buffer(0), index_buffer(0), update_id(0), last_frame(0) {}
    };
    ...
    struct TexturePrim {
        ...
        void alloc_texture(int w, int h, const RealTexFormat& form) {
            ...
            if (!(f.texture_format & kTextureDontMipmap)) {
                if (!(f.texture_format & kTextureDontFilterMipmap))
                    min_filter = !(f.texture_format & kTextureDontFilterPixels)
            ...
            mag_filter = !(f.texture_format & kTextureDontFilterPixels)
                             ? GL_LINEAR
                             : GL_NEAREST;
            ...
            glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                            f.texture_format & kTextureClamp ? GL_CLAMP_TO_EDGE
                                                             : GL_REPEAT);
            glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                            f.texture_format & kTextureClamp ? GL_CLAMP_TO_EDGE
                                                             : GL_REPEAT);
            glTexImage2D(GL_TEXTURE_2D, 0, f.internal_format, w, h, 0, f.format,
                         f.type, NULL);
        }
        ...
        void upload_texture_data(int w, int h, void* data) {
            ...
            glBindTexture(GL_TEXTURE_2D, tex);
            if (data)
                glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w, h, f.format, f.type,
                                data);
  ...
  public:
    ...
    void bind_color_texture(VirtualResource t, int level, int buffer) {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, render_tex_frame_buffer);
#if USE_OPENGLES
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + buffer,
                               GL_TEXTURE_2D, textures[t].tex, level);
#else
        glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + buffer,
                             textures[t].tex, level);
#endif
        GLenum g = glCheckFramebufferStatus(GL_FRAMEBUFFER);

        if (g != GL_FRAMEBUFFER_COMPLETE)
            PLOGE << "FBO is not complete error:" << std::hex << g;
        current_frame_buffer = render_tex_frame_buffer;
    ...
    void set_shader(Renderable*, VirtualResource& x, Uniforms** uniforms,
                    int size);
    void add_format(unsigned int base_format, GLenum internal_format,
    ...
    RealTexFormat get_closest_format(int format) {
        ...
        for (size_t x = 0; x < tex_formats.size(); ++x) {
            if (texture_format_fitness(format, ret.texture_format) <
                texture_format_fitness(format, tex_formats[x].texture_format)) {
                ret = tex_formats[x];
            }
        }
        if ((ret.texture_format & kTextureAnsiotropicMask) <
            ...
            format = (format & ~kTextureAnsiotropicMask) |
                     (ret.texture_format & kTextureAnsiotropicMask);
        ret.texture_format =
            (ret.texture_format &
             ~(kTextureFlagMask | kTextureAnsiotropicMask)) |
            (format & (kTextureFlagMask | kTextureAnsiotropicMask));
    ...
    void draw_vertex_array(std::shared_ptr<VertexArray> array, int end_vert,
                           int d);
