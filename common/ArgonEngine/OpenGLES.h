...
class OpenGLES : public RenderAPI {
    struct vert_data {
        ...
        size_t last_frame;
        vert_data()
            : vert_buffer(0), index_buffer(0), update_id(0),
              last_frame(0) {}
    };
    ...
    struct TexturePrim {
        ...
        size_t last_frame;
        TexturePrim()
            : width(-1), height(-1), last_frame(0) {
            glGenTextures(1, &tex);
        }
        void alloc_texture(int w, int h,
                           const RealTexFormat& form) {
            ...
            if (!(f.texture_format & kTextureDontMipmap)) {
                ...
                min_filter = !(f.texture_format &
                               kTextureDontFilterPixels)
                                 ? GL_LINEAR
                                 : GL_NEAREST;

            mag_filter = !(f.texture_format &
                           kTextureDontFilterPixels)
                             ? GL_LINEAR
                             : GL_NEAREST;
            if (f.texture_format & kTextureAnsiotropic2x)
                glTexParameterf(
                    GL_TEXTURE_2D,
                    GL_TEXTURE_MAX_ANISOTROPY_EXT, 2);
            else if (f.texture_format &
                     kTextureAnsiotropic4x)
                glTexParameterf(
                    GL_TEXTURE_2D,
                    GL_TEXTURE_MAX_ANISOTROPY_EXT, 4);
            else if (f.texture_format &
                     kTextureAnsiotropic8x)
                glTexParameterf(
                    GL_TEXTURE_2D,
                    GL_TEXTURE_MAX_ANISOTROPY_EXT, 8);
            ...
            glTexParameterf(GL_TEXTURE_2D,
                            GL_TEXTURE_WRAP_S,
                            f.texture_format & kTextureClamp
                                ? GL_CLAMP_TO_EDGE
                                : GL_REPEAT);
            glTexParameterf(GL_TEXTURE_2D,
                            GL_TEXTURE_WRAP_T,
        ...
        void upload_texture_data(int w, int h, void* data) {
            ...
            if (data)
                glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, w,
                                h, f.format, f.type, data);

            if (!(f.texture_format & kTextureDontMipmap)) {
                glGenerateMipmap(GL_TEXTURE_2D);
            }
        }
        ~TexturePrim() { glDeleteTextures(1, &tex); }
    };
    struct shader_data {
        GLuint program;
    ...
    typedef std::map<StringIntern, Uniform>::iterator
        u_iter;
    ...
    std::map<std::shared_ptr<VertexArray>, vert_data>
        vertex_arrays;
    ...
    bool initialized;
    bool compile_shader(GLuint& shader, GLenum type,
    ...
    void bind_attributes(shader_data& data,
                         std::shared_ptr<VertexArray> v);

  public:
    OpenGLES() : initialized(false) { init_fbos(); }
    void cache_array(std::shared_ptr<VertexArray> array);
    void cache_material(Material& state,
                        const VirtualResource& shader);
    ...
    void discard_buffers(bool color, bool depth,
                         bool stencil) {
        if (extensions["GL_EXT_discard_framebuffer"]) {
            std::vector<GLenum> v;
            if (color)
                v.push_back(GL_COLOR_ATTACHMENT0);
            if (depth)
                v.push_back(GL_DEPTH_ATTACHMENT);
            if (stencil)
                v.push_back(GL_STENCIL_ATTACHMENT);
    ...
    void bind_depth_texture(VirtualResource t) {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER,
                          render_tex_frame_buffer);
#if USE_OPENGLES
        glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
            GL_TEXTURE_2D, textures[t].tex, 0);
#else
        ...
        if (g != GL_FRAMEBUFFER_COMPLETE)
            PLOGE << "FBO is not complete error:" << g;
        current_frame_buffer = render_tex_frame_buffer;
        GL_CHECK("AfterDepthTexBind");
    }
    void bind_color_texture(VirtualResource t, int level,
                            int buffer) {
...
#if USE_OPENGLES
        glFramebufferTexture2D(
            GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + buffer,
            GL_TEXTURE_2D, textures[t].tex, level);
#else
        glFramebufferTexture(GL_FRAMEBUFFER,
                             GL_COLOR_ATTACHMENT0 + buffer,
                             textures[t].tex, level);
#endif
        GLenum g = glCheckFramebufferStatus(GL_FRAMEBUFFER);

        if (g != GL_FRAMEBUFFER_COMPLETE)
            PLOGE << "FBO is not complete error:"
                  << std::hex << g;
        current_frame_buffer = render_tex_frame_buffer;
    ...
    void set_shader(Renderable*, VirtualResource& x,
                    Uniforms** uniforms, int size);
    void add_format(unsigned int base_format,
    ...
    RealTexFormat get_closest_format(int format) {
        ...
        for (size_t x = 0; x < tex_formats.size(); ++x) {
            if (texture_format_fitness(format,
                                       ret.texture_format) <
                texture_format_fitness(
                    format,
                    tex_formats[x].texture_format)) {
                ret = tex_formats[x];
            }
        }
        if ((ret.texture_format & kTextureAnsiotropicMask) <
            (format & kTextureAnsiotropicMask))
            format = (format & ~kTextureAnsiotropicMask) |
                     (ret.texture_format &
                      kTextureAnsiotropicMask);
        ret.texture_format =
            (ret.texture_format &
             ~(kTextureFlagMask |
               kTextureAnsiotropicMask)) |
            (format &
             (kTextureFlagMask | kTextureAnsiotropicMask));

        return ret;
    }
    void set_render_flags(unsigned int render_flags) {
        unsigned int delta =
            render_flags ^ current_render_flags;
        ...
        if (delta & kRenderDepthMask)
            glDepthMask(
                bool(render_flags & kRenderDepthMask));
    ...
    void
    draw_vertex_array(std::shared_ptr<VertexArray> array,
                      int end_vert, int d);
