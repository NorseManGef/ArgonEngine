...
const float kGlyphRFactor =
    kGlyphRenderRes / float(kGlyphHeight);

const int kGlyphCacheSize =
    (kGlyphTextureSize / kGlyphWidth) *
    (kGlyphTextureSize / kGlyphHeight) * 4;
...
struct GlyphCache {
    ...
    Argon::PriorityCache<Glyph, GlyphKey, kGlyphCacheSize>
        glyphs;

    GlyphCache();
    inline void set_val(const int x, int y,
                        unsigned char* array);
    unsigned char interpolate(float x, float y,
                              unsigned char* bu);
    void build_distance_field(uint8_t* bu, int pos);
    Glyph* get_glyph(Argon::VirtualResource font,
                     int glyph);
};
struct Font : public Argon::VirtualResourceIMPL::Data {
    ...
    virtual float get_advance(int last_glyph,
                              int glyph) = 0;
    ...
    virtual bool
    reload(Argon::VirtualResourceIMPL::Source* s) = 0;
...
struct Label : public Renderable {
    ...
    /*!
        ...
        \param s whether to align the text to the center,
       left, or right; defaults to left
        \remarks should only be used for initialization or
       change
    */
    void render_str(const char* string,
                    Argon::VirtualResource font,
                    LineStyle s = kLineLeft);
