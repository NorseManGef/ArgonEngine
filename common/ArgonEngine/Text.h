...
const int kGlyphCacheSize =
    (kGlyphTextureSize / kGlyphWidth) * (kGlyphTextureSize / kGlyphHeight) * 4;
...
struct GlyphCache {
    ...
    struct GlyphKey {
        ...
        bool operator!=(const GlyphKey& k) const {
            return resource != k.resource || glyph != k.glyph;
        }
        bool operator>(const GlyphKey& k) const {
            return resource != k.resource ? resource > k.resource
                                          : glyph > k.glyph;
        }
        bool operator<(const GlyphKey& k) const {
            return resource != k.resource ? resource < k.resource
...
struct Label : public Renderable {
    ...
    /*!
       ...
       change
    */
    void render_str(const char* string, Argon::VirtualResource font,
