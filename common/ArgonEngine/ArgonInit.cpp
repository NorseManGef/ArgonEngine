...
static void InitVirtual(std::string organization_name, std::string app_name) {
    ...
    VirtualResource::all_sources()["document:"] =
        new VirtualResourceIO(doc_dir, true);
    VirtualResource::all_sources()["resource:"] =
        new VirtualResourceIO(base + "resources", false);
    VirtualResource::all_sources()["shader:"] =
        new VirtualResourceIO(base + "shaders", false);
    VirtualResource::all_sources()["user:"] =
        VirtualResource::all_sources()["home:"] =
            new VirtualResourceIO(home, true);
    VirtualResource::all_sources()["save:"] =
        VirtualResource::all_sources()["app-data:"] =
            VirtualResource::all_sources()["pref:"] =
                new VirtualResourceIO(pref_dir, true);
    VirtualResource::all_sources()["cache:"] =
        VirtualResource::all_sources()["tmp:"] =
            new VirtualResourceIO(tmp_dir, true);
...
void terminate_engine() {
    if (context) {
        /* SDL_GL_MakeCurrent(0, NULL); */ /* doesn't
                                              do
                                              anything
                                            */
...
void initialize_engine(std::string organization_name, std::string app_name,
    ...
    try {
        ...
        // This is in the init function before
        // anything is initialized, so it should
        // be safe to just exit from these
        // exception handlers.
    } catch (args::Help) {
    ...
    if (!win) {
        PLOGF << "SDL_CreateWindowWithProperties "
                 "failed: "
              << SDL_GetError();
        terminate_engine();
    }

    // Create an OpenGL context associated with
    // the window. SDL_GLContext glcontext =
...
uint32_t sdl_key_to_argon(SDL_Keycode key) {
    ...
    case SDLK_KP_DECIMAL:
        return kInputIDKPDecimal;
    case SDLK_KP_DIVIDE:
        return kInputIDKPDivide;
    case SDLK_KP_MULTIPLY:
        return kInputIDKPMultiply;
    case SDLK_KP_MINUS:
        return kInputIDKPSubtract;
    case SDLK_KP_PLUS:
        return kInputIDKPAdd;
    case SDLK_KP_ENTER:
        return kInputIDKPEnter;
    case SDLK_KP_EQUALS:
        return kInputIDKPEqual;

    case SDLK_LSHIFT:
        return kInputIDLeftShift;
    case SDLK_RSHIFT:
        return kInputIDRightShift;

    case SDLK_LALT:
        return kInputIDLeftAlt;
    case SDLK_RALT:
        return kInputIDRightAlt;

    case SDLK_LCTRL:
        return kInputIDLeftControl;
    case SDLK_RCTRL:
        return kInputIDRightControl;

    case SDLK_LGUI:
        return kInputIDLeftSuper;
    case SDLK_RGUI:
        return kInputIDRightSuper;
    ...
    case SDLK_DOWN:
        return kInputIDDown;
    case SDLK_RIGHT:
        return kInputIDRight;
    case SDLK_LEFT:
        return kInputIDLeft;

    case SDLK_ESCAPE:
        return kInputIDEscape;
    case SDLK_RETURN:
        return kInputIDEnter;
    case SDLK_TAB:
        return kInputIDTab;
    case SDLK_BACKSPACE:
        return kInputIDBackspace;
    case SDLK_INSERT:
        return kInputIDInsert;
    case SDLK_DELETE:
        return kInputIDDelete;

    case SDLK_PAGEUP:
        return kInputIDPageUp;
    case SDLK_PAGEDOWN:
        return kInputIDPageDown;
    case SDLK_HOME:
        return kInputIDHome;
    case SDLK_END:
        return kInputIDEnd;
    case SDLK_CAPSLOCK:
        return kInputIDCapsLock;
    case SDLK_SCROLLLOCK:
        return kInputIDScrollLock;
    case SDLK_NUMLOCKCLEAR:
        return kInputIDNumLock;
    case SDLK_PRINTSCREEN:
        return kInputIDPrintScreen;
    case SDLK_PAUSE:
        return kInputIDPause;
    }
...
void handle_window_event(SDL_Event& e) {
    ...
    default:
        PLOGW << "In function "
                 "handle_window_event(): invalid "
                 "SDL_Event type "
                 "(not of SDL_EVENT_WINDOW "
                 "variety). Event: "
              << e.window.type;
        break;
    }
...
bool handle_event(void* userdata, SDL_Event* event) {
    ...
    // Handle events
    {
        ...
        if (e.type == SDL_EVENT_QUIT) {
            ...
            if (!j)
                j = SDL_OpenJoystick(e.jdevice.which);
...
bool poll_events() {
    ...
    if (last_screen != Screen::logical_size) {
        ...
        SDL_SetWindowSize(win, Screen::logical_size[0],
                          Screen::logical_size[1]);
        last_screen = Screen::logical_size;
...
void swap_buffers() {
    ...
    // Linear IFR based accumulator to approximate
    // frame time.
