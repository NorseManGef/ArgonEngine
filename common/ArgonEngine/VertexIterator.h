...
/**
 * @brief This file provides an implementation of a generic
 * iterator for VertexArrays.
 * @author Skyler Saleh
...
namespace Argon {
struct VertexArray;
//! @brief Contains the information needed to manipulate an
//! attributes data.
struct VertexAttribPair : public ReflectionBase {
    ...
    //! The name of the attribute. This is how you will
    //! access the data in a shader.
    StringIntern attribute;
    size_t stride;
    //! The components that the attribute contains.
    int components;
    //! A function that can get component 'x' of the data
    ...
    MAKE_VISIT(VertexAttribPair, {
        ...
        if (t.attribute == "position")
            t.attribute = kPositionAttribute;
        else if (t.attribute == "normal")
            t.attribute = kNormalAttribute;
    });
};
//! @brief This class provides a means of iterating through
//! the arbitrary vertex data stored in a VertexArray.
//! @details A Vertex Iterator will only iterate through a
//! single attribute of data at a time, and it handles the
//! proper conversion to and from the different data types
//! stored in the array. It is not valid if the array
//! changes size or attributes.
class VertexIterator {
    uint8_t* data; //!< A pointer to the raw vertex data in
                   //!< the array.
    VertexAttribPair* attr;
    //! A class that mimics raw access to the individual
    //! vertices and passes it on to the VertexIterator.
    struct IterHelper {
  ...
  public:
    VertexIterator(uint8_t* data, VertexAttribPair* attr)
        : attr(attr), data(data) {}
    VertexIterator() : data(NULL), attr(NULL) {}

    //! Returns true if VertexIterator iter points to the
    //! same vertex.
    bool operator==(const VertexIterator& iter) const {
        return data == iter.data;
    }
    //! Returns true if VertexIterator iter does not point
    //! to the same vertex.
    bool operator!=(const VertexIterator& iter) const {
        return data != iter.data;
    }
    //! Returns true if VertexIterator iter points to a
    //! lower vertex.
    bool operator<(const VertexIterator& iter) const {
        return data < iter.data;
    }
    ...
    IterHelper set(float value) {
        for (size_t x = 0; x < attr->components; ++x)
            attr->from_float(data, x, value);
        return IterHelper(*this);
    }
    //! Returns the current vertices data.
    IterHelper operator*() { return IterHelper(*this); }
    //! Returns true if the current vertex is less than a
    //! value.
    bool less(float value) {
        bool b = false;
        for (size_t x = 0; x < attr->components; ++x)
            b |= attr->to_float(data, x) < value;
        return b;
    }
    ...
    //! Returns true if the current vertex's value is less
    //! the vector 'v'.
    template <typename T, size_t comps>
    ...
    //! Returns true if the current vertex's value is equal
    //! to the vector 'v'.
    template <typename T, size_t comps>
    ...
    //! Returns true if the current vertex's values are
