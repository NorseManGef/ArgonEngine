...
void* deffered_loading(void* data_pt) {
    ...
    if (!image->source || !image->source->reload())
        return NULL;
...
unsigned int VirtualResourceImage::parse_format(
    std::map<std::string, std::string>& arg_map) const {
    std::string& f = arg_map["f"];
    unsigned int format = kTextureRGBA8;
    if (f == "RGB565")
        format = kTextureRGB565;
    else if (f == "RGB8")
        format = kTextureRGB8;
    else if (f == "RGBF16")
        format = kTextureRGBF16;
    else if (f == "RGBF32")
        format = kTextureRGBF32;
    else if (f == "RGBF64")
        format = kTextureRGBF64;

    else if (f == "RGBA5551")
        format = kTextureRGBA5551;
    else if (f == "RGBA4")
        format = kTextureRGBA4;
    else if (f == "RGBA8")
        format = kTextureRGBA8;
    else if (f == "RGBAF16")
        format = kTextureRGBAF16;
    else if (f == "RGBAF32")
        format = kTextureRGBAF32;
    else if (f == "RGBAF64")
        format = kTextureRGBAF64;

    else if (f == "D16")
        format = kTextureDepth16;
    else if (f == "D24")
        format = kTextureDepth24;
    else if (f == "D32")
        format = kTextureDepth32;

    else if (f == "S8")
        format = kTextureStencil8;
    else if (f == "DS")
        format = kTextureDepthStencil;
    else
        PLOGE << "Unknown tex format: " << f;
    ...
    if (ansio >= 8)
        format |= kTextureAnsiotropic8x;
    else if (ansio >= 4)
        format |= kTextureAnsiotropic4x;
    else if (ansio >= 1)
        format |= kTextureAnsiotropic2x;

    if (clamp)
        format |= kTextureClamp;
    if (!mip)
        format |= kTextureDontMipmap;
    if (!filter)
        format |= kTextureDontFilterPixels;
    if (!filter_mip)
        format |= kTextureDontFilterMipmap;
    if (fbo)
        format |= kTextureFBO;
    return format;
}
