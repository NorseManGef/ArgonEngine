...
//
//  Created by Skyler Saleh on 10/13/12.
...
bool PrintVisitor::is_array(size_t start) {
    ...
    for (; start < strings.size(); ++start) {
        ...
        if (strings[start] == "*MAPB*")
            recurse++;
        if (strings[start] == "*MAPE*")
            recurse--;
        if (recurse < 0)
            return complete == 0;
        if (complete == 2)
...
void PrintVisitor::print_string(std::ostream& s, size_t x,
                                size_t indent_level) {
...
bool JsonReader::handle_double(double& v) {
    ...
    if (val.isConvertibleTo(Json::realValue))
        v = val.asDouble();
    else if (val.isConvertibleTo(Json::intValue))
        v = val.asInt();
    else if (val.isString()) {
        v = Argon::string_to_number(val.asString());
    } else
...
struct AHFReader : public Visitor {
    struct StackData {
        ...
        unpack_int(intmax_t& val,
            ...
            case kUInt8:
                return read_buffer(stream, (char*)&val, 1);
            case kUInt16:
                return read_buffer(stream, (char*)&val, 2);
            case kUInt32:
                return read_buffer(stream, (char*)&val, 4);
            case kInt64:
            case kUInt64:
    ...
    bool can_unpack_value() override {
        if (stack.empty() || error)
            ...
            return stack.back()->can_unpack_value();
    }
...
bool Visitor::handle_double(double& v) {
    std::stringstream s;
    s << v;
...
bool AHFWriter::handle_string(std::string& key) {
    return stack->back().pack_string(key);
}
bool AHFWriter::begin_map() {
    stack->push_back(WriterStackData());
    return true;
}
...
bool AHFReader::begin_map() {
    uint64_t off = 0;
    if (stack.size()) {
        if (!stack.back()->key_data) {
            return false;
        };
        off = stack.back()->key_data->current_off;
        if (stack.back()->key_data->type != kAHF ||
            stack.back()->key_data->current_off >=
                stack.back()->key_data->end) {
            return false;
        }
    }
    ...
    if (stack.back()->unpack_map(source, off))
        return true;
