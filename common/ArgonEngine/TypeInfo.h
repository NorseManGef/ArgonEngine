...
/**
 * @brief This file contains the classes necessary to define
 * a classes type information for use with the Reflection
 * System.
 * @file TypeInfo.h
 * @author Skyler Saleh
 **/
...
#include <type_traits>
/**
 * @brief This macro allows you to quickly define the
 * TypeInfo of most structs.
 * @details
 * To use a struct with the Reflection System, its TypeInfo
 * must first be specified. This macro allows you to do that
 * easily. However, you may have to define your own TypeInfo
 * class for more advanced functionality.
 *
 * Here is an example of its use, to describe the following
 * classes:
 *
 * \code{.cpp}
 * struct MyClass{
 ...
 * \endcode
 *
 * You must add the MAKE_TYPE_INFO macro after the classes
 * declaration like so:
 * \code{.cpp}
 * struct MyClass{
 *     int x;
 ...
 *     if(!mode)t.recalculate_derived();//Derived must be
 * recalculated after unpacking.
 * };
 * struct MyClass2: public MyClass{
 *      int data;
 * };
...
//! Used to add a variable in the MAKE_TYPE_INFO macro under
//! a certain key.
...
struct Visitor;
/**
 * @brief This class represents an objects complete
 * typeinfo.
 * @details
 * A specialization of this class must provide an enum where
 * valid is set to one and a unpack and pack static
 * functions. The Unpack and Pack Static Functions should
 * each take a refrerence to the type the typeinfo is
 * describing along with a reference to an Argon::Visitor.
 * It should call the respective pack and unpack functions
 * on the visitor for every variable required to serialize
 * or deserialize a class.
 *
 * You should use this class over TypeInfoEasy when you must
 * provide specialized functionality for unpack/pack such as
 ...
 *      enum{valid=1};
 *      static void pack(const MyClass& obj, Argon::Visitor
 * &vis){ if(vis.pack_map()){ vis.pack(obj.x,"x");
 *              vis.pack(obj.y,"y");
...
struct ReflectionBase {
    ...
    static void print_registered_factories() {
        IF_PLOG(plog::info) {
            ...
            stream << "There are " << factory_map().size()
                   << " registered factories, that the "
                      "reflection system can spawn:\n";
            size_t max = 0;
...
/**
 * @brief The Visitor class is the key external class of the
 * Reflection System.
 * @details
 * The Visitor class gets called on all variables of a class
 * that needs to be serialized by the user or TypeInfo
 * specialized classes.
 **/
struct Visitor {
    ...
    bool error; //!< True if an error has occurred false
                //!< otherwise.
    //! A function pointer that returns the name of a type
    //! currently being described.
    const char* type_name;
    Visitor() : error(false) {}
    //! Sets the key to identify a piece of data by when
    //! packing or unpacking. Returns true if successful.
    virtual bool set_key(const char* key) = 0;

    //! Handles an integer. Call this function multiple
    //! times to pack an array. Returns true if successful.
    ...
    //! Handles a double. Call this function multiple times
    //! to pack an array. Returns true if successful.
    virtual bool handle_double(double& v);

    //! Handles a string. Call this function multiple times
    //! to pack an array. Returns true if successful.
    virtual bool handle_string(std::string& key) = 0;
    //! Marks the beginning of a new map of data to pack.
    //! You must call this in each typeinfo that identifies
    //! data by a key. You also must check that the return
    //! type is true before packing data or calling finish
    //! map on the visitor. Returns true if successful.
    virtual bool begin_map() = 0;

    //! Marks the end of a map of data. There must be the
    //! same amount of finish map calls as successful
    //! pack_map or unpack_map calls. Returns true on
    //! success.
    virtual bool finish_map() = 0;
    //! Returns true if their is another value that can be
    ...
    virtual bool has_unhandled_keys() { return false; }
    //! Returns a std::string that contains one key that has
    //! not been handled.
    virtual std::string get_unhandled_key() { return ""; };
     ...
     * @tparam mode Whether to pack or unpack the variable.
     * True to pack, False to unpack.
...
/**
 * @brief This class provides an implementation of an
 * TypeInfo class that allows you to combine your pack and
 * unpack functions into one function called handle. It
 * allows you to have less duplication in your type info
 * declarations.
 *
 * @details The class should be subclassed when defining a
 * TypeInfo. It is used like this:
 *
 * \code{.cpp}
 * struct MyClass{
 *      int x;
 ...
 *      void recalculate_derived(){derived=x-y;}
 * };
 * template<> struct Argon::TypeInfo<MyClass>: public
 * Argon::TypeInfoEasy<Argon::TypeInfo<MyClass>,MyClass>{
 *      enum{valid=1};
 *      template <bool mode>
 ...
 *          if(!mode)obj.recalculate_derived(); //Derived
 * must be recalculated after unpacking.
 *      }
 * };
 * \endcode
 *
 * @tparam T The TypeInfo class that is subclassing the
 * TypeInfoEasy struct.
