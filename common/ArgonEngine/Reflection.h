...
struct VisitableResource
    ...
    virtual void
    read_visitor_finish(std::shared_ptr<Visitor>& v) {}
    virtual void
    write_visitor_finish(std::shared_ptr<Visitor>& v) {}
...
struct JsonResource : public VisitableResource {
    ...
    virtual std::shared_ptr<Visitor>
    read_visitor() override;
...
struct PrintVisitor : public Visitor {
    ...
    PrintVisitor(bool print_long_strings = false)
        : print_long_strings(print_long_strings) {}
    ...
    void print_string(std::ostream& s, size_t x,
                      size_t indent_level);
    bool begin_map() {
        strings.push_back("*MAPB*");
        strings.push_back(type_name);
        return true;
    }
    bool finish_map() {
        strings.push_back("*MAPE*");
        return true;
    }
    ...
    void print(std::ostream& s, size_t& x,
               size_t indent_level = 0, int recurse = 9);
};

} // namespace Argon

inline static std::ostream&
operator<<(std::ostream& s, Argon::PrintVisitor& vis) {
    size_t x = 0;
    vis.print(s, x);
    return s;
}
...
->typename std::enable_if<Argon::TypeInfo<T>::valid,
std::ostream&>::type{ Argon::PrintVisitor vis;
    vis.handle(v);
    return s<<vis;
}*/
