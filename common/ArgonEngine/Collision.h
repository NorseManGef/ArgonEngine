...
#include "Vector.h"
namespace Argon {
// Returns the point closest to "point" on the line
// containing point "lineA" and point "lineB" If the line is
...
closest_point_on_line(const VectorBase<T, TComps>& lineA,
    ...
    if (clamp_a && delta < 0.0)
        delta = 0.0;
    if (clamp_b && delta > 1.0)
        delta = 1.0;
    return lineA + ab * delta;
}
template <typename T>
bool ray_intersects_triangle(const VectorBase<T, 3>& v0,
...
template <typename T>
T point_plane_distance(VectorBase<T, 3> plane, T distance,
                       VectorBase<T, 3> point) {
