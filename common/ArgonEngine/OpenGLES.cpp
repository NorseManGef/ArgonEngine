...
void OpenGLES::bind_attributes(shader_data& d, std::shared_ptr<VertexArray> v) {
    ...
    while (at_it != d.attributes.end()) {
        ...
        if (it == v->attributes.end())
            ...
            GLenum t = GL_FLOAT;
            switch (it->type) {
            case Argon::kRenderTypeUByte:
                t = GL_UNSIGNED_BYTE;
                break;
            case Argon::kRenderTypeShort:
                t = GL_SHORT;
                break;
            case Argon::kRenderTypeFloat:
                t = GL_FLOAT;
                break;
            case Argon::kRenderTypeByte:
                t = GL_BYTE;
                break;
            default:
                break;
...
void OpenGLES::init_texture_formats() {
    ...
    if (extensions["GL_EXT_texture_filter_anisotropic"]) {
        ...
        if (v >= 8.)
            texture_mask |= kTextureAnsiotropic8x;
        else if (v >= 4.)
            texture_mask |= kTextureAnsiotropic4x;
        else if (v >= 2.)
            texture_mask |= kTextureAnsiotropic2x;
    }
...
GLenum blend_converter(unsigned int b) {
    ...
    case kDstAlpha & 0xF:
        return GL_DST_ALPHA;
    case kOneMinusDstColor & 0xF:
        return GL_ONE_MINUS_DST_COLOR;
    case kOneMinusDstAlpha & 0xF:
        return GL_ONE_MINUS_DST_ALPHA;
    case kSrcColor & 0xF:
        return GL_SRC_COLOR;
    case kSrcAlpha & 0xF:
        return GL_SRC_ALPHA;
    case kOneMinusSrcColor & 0xF:
        return GL_ONE_MINUS_SRC_COLOR;
    case kOneMinusSrcAlpha & 0xF:
        return GL_ONE_MINUS_SRC_ALPHA;

    case kConstantColor & 0xF:
        return GL_CONSTANT_COLOR;
    case kOneMinusConstantColor & 0xF:
        return GL_ONE_MINUS_CONSTANT_COLOR;
    case kConstantAlpha & 0xF:
        return GL_CONSTANT_ALPHA;
    case kOneMinusConstantAlpha & 0xF:
        return GL_ONE_MINUS_CONSTANT_ALPHA;

    case kSrcAlphaSaturate & 0xF:
...
void OpenGLES::set_uniforms(Uniforms** all_uniforms, int size) {
    ...
    while (it != current_shader->uniforms.end()) {
        ...
        break;
        case GL_INT:
            SET_UNIFORM_COPY2(f, GLint, glUniform1iv, GLint)
            break;
        case GL_INT_VEC2:
            SET_UNIFORM_COPY(f2, gliv2, glUniform2iv, GLint)
            break;
        case GL_INT_VEC3:
            SET_UNIFORM_COPY(f3, gliv3, glUniform1iv, GLint)
            break;
        case GL_INT_VEC4:
            SET_UNIFORM_COPY(f4, gliv4, glUniform1iv, GLint)
            break;
#ifndef USE_OPENGLES
        case GL_BOOL:
...
#endif
        case GL_FLOAT:
            SET_UNIFORM(f, glUniform1fv, GLfloat);
            break;
        case GL_FLOAT_VEC2:
            SET_UNIFORM(f2, glUniform2fv, GLfloat);
            break;
        case GL_FLOAT_VEC3:
            SET_UNIFORM(f3, glUniform3fv, GLfloat);
            break;
        case GL_FLOAT_VEC4:
...
bool OpenGLES::compile_shader(GLuint& shader, GLenum type,
...
int OpenGLES::make_shader(VirtualResource& shader) {
    ...
    if (v)
        glDetachShader(s.program, v);
    if (f)
        glDetachShader(s.program, f);
    if (f)
        glDeleteShader(f);
    if (v)
        glDeleteShader(v);
