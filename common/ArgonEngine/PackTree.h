...
//  Neon Rush
//
//  Created by Skyler Saleh on 7/4/11.
...
#define ARPackTree_h
namespace Argon {
template <typename T> struct PackTree {
    T x;
    T y;
    T width;
    T height;
    ...
    bool has_children() { return child; }
    PackTree* pack_node(T w, T h) {
        if (width < w || height < h || leaf)
            return NULL;
        if (has_children()) {
            PackTree* t = smallest_fit(w, h);
            if (t)
                return t->pack_node(w, h);
            return NULL;
        }
    ...
    bool empty() { return !has_children() && !leaf; }
