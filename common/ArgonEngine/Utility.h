...
/**
 * @brief Provides some Utility classes for the Argon Engine
 * such as OrderedVector, FixedVector,
 * ObjectTracer/ObjectTracker, and StringIntern.
 * @file Utility.h
 * @author Skyler Saleh
 **/
#ifndef Utility_h
    ...
    while (i != end && state != 10000) {
        ...
        case 11:
            ...
            case 'N':
                ...
                state = 10000;
                break;
                // Don't need to check rest because function
...
/**
 * @brief This class implements a sorted std::vector. It
 * operates like an std::set, however iterating through all
 * elements is fast, and insertion is expensive, and changes
 * the memory location of its contents. However all elements
 * are sorted like in a set.
...
template <class T> class InstanceList {
    ...
    void _remove() {
        mutex().lock();
        if (next_instance)
            next_instance->prev_instance = prev_instance;
        if (prev_instance)
            prev_instance->next_instance = next_instance;
        if (get_root() == static_cast<T*>(this))
            get_root() = next_instance;
        mutex().unlock();
    }
...
template <typename T> struct NoAutoAssignPtr {
    T* ptr;
...
/**
 * @brief This class implements String Interning. It allows
 * for fast comparisons, and copies. However mutations of
 * the string are expensive.
 * @details
 * In String Interning every unique string gets its own
 * number. This makes string comparison and copies an O(1)
 * operation, and very fast. However, since a string must be
 * compared with all other StringInterns when it is
 * modified, its creation and mutation are expensive. This
 * class is useful, when you have mostly static strings that
 * need to be looked up, or compared against many times.
 * StringInterns also do not compare to each other
 * alphabetically.
 **/
#if USE_STACKSTRING
...
struct PriorityCache {
    ...
    PriorityCache() {
        ...
        for (int x = 0; x < size; ++x) {
            ...
            if (x == size - 1)
                cache[x].more_fresh = NULL;
            if (x == 0)
                cache[x].more_stale = NULL;
        }
    };
